 ...
 *
 * Family-Captcha-Standard (D-LIN-OAK-TURNSTILE): von oakwood nach goldoni
 * portiert (2026-06-12). Turnstile ist der Anti-Spam-Layer auf beiden
 * Formularen (/kontakt + /feiern). Kostenlos + unbegrenzt (kein Free-Tier-
 * Limit), DSGVO-konform (kein Cookie, kein Cross-Site-Tracking),
 * non-interactive im Normalfall.
 *
 * Flow:
 *   1. Client-Widget (components/forms/Turnstile.tsx) lädt das Cloudflare-
 *      Script und injiziert das Ergebnis als hidden input
 *      `cf-turnstile-response` in die Form.
 *   2. Die Server Action (src/app/actions/inquiry.ts → sendInquiry) liest das
 *      Feld aus und ruft `verifyCaptchaToken(token)` auf.
 ...
 * default — keep in sync with the widget in components/forms/Turnstile.tsx.
 */
